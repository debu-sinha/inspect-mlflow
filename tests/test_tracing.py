...
import inspect_mlflow.tracing as _tracing_mod
...
async def test_run_end_with_exception_records_error(tracing_env):
    ...
    with patch.object(_tracing_mod, "mlflow") as mock_mlflow:
        ...
        await hook.on_run_start(RunStart(eval_set_id=None, run_id="run-fail", task_names=["t"]))
        await hook.on_run_end(
            RunEnd(
                eval_set_id=None,
                run_id="run-fail",
    ...
    with patch.object(_tracing_mod, "mlflow") as mock_mlflow:
        ...
        await hook.on_sample_event(
            SampleEvent(
                ...
                eval_id="eval-001",
                sample_id="s-001",
                event=SpanBeginEvent(id="span-inner", name="generate", parent_id="span-outer"),
            )
        )
